...
from .ppo import PPO
from .networks.discriminator import GAILDiscrim
...
class GAIL(PPO):
    ...
    def __init__(self, buffer_exp, image_shape, state_shape, action_shape, device, seed,
        ...
        super().__init__(
            image_shape, state_shape, action_shape, device, seed, gamma, rollout_length,
    ...
    def update(self, writer):
        ...
        for _ in range(self.epoch_disc):
            ...
            images, states, actions = self.buffer.sample(self.batch_size)[:3]
            # Samples from expert's demonstrations.
            images_exp, states_exp, actions_exp = \
                self.buffer_exp.sample(self.batch_size)[:3]
            # Update discriminator.
            self.update_disc(images, states, actions, images_exp,
                             states_exp, actions_exp, writer)

        # We don't use reward signals here,
        images, states, actions, _, dones, log_pis, next_images, next_states = self.buffer.get()

        # Calculate rewards.
        rewards = self.disc.calculate_reward(images, states, actions)

        # Update PPO using estimated rewards.
        self.update_ppo(
            images, states, actions, rewards, dones, log_pis, next_images, next_states, writer)

    def update_disc(self, images, states, actions, images_exp, states_exp, actions_exp, writer):
        # Output of discriminator is (-inf, inf), not [0, 1].
        logits_pi = self.disc(images, states, actions)
        logits_exp = self.disc(images_exp, states_exp, actions_exp)
